...
String logFileName;                              // sLog data to a file of this name when LOG_RAW_DATA_TO_SD is true.
File logFile;
...
bool initLIDARCom(bool triggeredMode = false)
{
  ...
  if (tfmP.sendCommand(HARD_RESET, 0))
  {
...
bool initLIDAR(bool triggeredMode = false)
{
  if (initLIDARCom(triggeredMode))
  {
    ...
    DEBUG_PRINT("    Initial LIDAR Reading: ");
    bool result;
    int16_t dist = 0;
  ...
  {
    DEBUG_PRINTLN("ERROR! LIDAR Sensor not found or sensor error.");
...
void showLIDARDistanceHistogram()
{
  DEBUG_PRINTLN(getDistanceHistogramString());
}
...
String getDistanceHistogramChartString(Config config)
{
  ...
  {
    if (lidarDistanceHistogram[i] < minValue)
      minValue = lidarDistanceHistogram[i];
    if (lidarDistanceHistogram[i] > maxValue)
      maxValue = lidarDistanceHistogram[i];
  }
  ...
  if (maxValue > 0)
  {
    ...
    {
      if ((i % 5) == 0)
      { // Put a tic on the axis every 50 cm.
...
int processLIDARSignal(Config config)
{
  ...
  /*
    DEBUG_PRINT(lidarResult);
    DEBUG_PRINT(" ");
    DEBUG_PRINT(tfmP.status);
    DEBUG_PRINT(" ");
    DEBUG_PRINTLN(tfDist);
  */
...
void logToSDCard()
{
  ...
  else
  {
    ...
    // Close the file
    // DEBUG_PRINTLN(String(millis())+","+String(tfDist));
